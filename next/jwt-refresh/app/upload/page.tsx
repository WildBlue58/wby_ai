...
const Upload = () => {
  ...
  const totalChunks = useMemo(() => {
    return Math.ceil(file?.size ?? 0 / CHUNK_SIZE);
  }, [file]);
        ...
        {file && (
          <div className="rounded-xl border bg-white p-4 shadow">
            ...
            <div className="mt-2 text-sm text-gray-500">
              分片大小:{CHUNK_SIZE / (1024 * 1024)}MB 分片总数:{totalChunks}
            </div>
            ...
            <div className="mt-4 flex gap-2">
              <button
                className="rounded-xl bg-black px-4 py-2 text-white disables:opacity-50"
                disabled={!file}
                onClick={startUpload}
              >
                开始上传
